...
@file tests/unit/script/test_odometry_one_meter_test.py
"""
...
SCRIPT_PATH = PROJECT_ROOT / "src" / "script" / "test" / "odometry_one_meter.py"


def load_test_script_module():
    """按文件路径加载里程计一米标定脚本."""

    spec = spec_from_file_location("odometry_one_meter_test_script", SCRIPT_PATH)
...
def test_distance_calibration_resets_runs_y_positive_target_and_stops() -> None:
    ...
    result = module.run_one_meter_calibration(
        car,
    ...
    assert car.events[1][1] == pytest.approx(0.0)
    assert car.events[1][2] == pytest.approx(module.TARGET_DISTANCE_M)
    ...
    expected_detail = (
        "result=done target_y=%.3f x=0.000 y=0.100 err_x=0.000 err_y=%.3f"
