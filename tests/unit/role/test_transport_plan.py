...
def _brick(edge, left, right):
    return ("brick", edge, left, right)


def test_startup_target_stops_at_nearest_left_obstacle_boundary() -> None:
    """出库第一段只受最近 left 障碍的原始边界限制."""
    slots = (
        _brick(FIELD_EDGE_BOTTOM, 0.2, 0.4),
        _brick(FIELD_EDGE_LEFT, 0.45, 0.8),
        _brick(FIELD_EDGE_RIGHT, 0.3, 0.6),
    )
...
def test_transport_heading_merges_touching_expanded_intervals() -> None:
    ...
    slots = (
        _brick(FIELD_EDGE_TOP, first_left, first_right),
        _brick(FIELD_EDGE_TOP, second_left, second_right),
        _brick(FIELD_EDGE_RIGHT, FIELD_HEIGHT_M * 0.25, FIELD_HEIGHT_M * 0.50),
    )
...
@pytest.mark.parametrize(
    ("position_x", "expected_heading"),
    ((1.5, -20.0), (1.7, 20.0)),
)
def test_transport_heading_applies_edge_minimum_avoidance_angle(
    monkeypatch, position_x: float, expected_heading: float
) -> None:
    monkeypatch.setattr(
        motion_params,
        ...
        {
            FIELD_EDGE_TOP: 20.0,
            FIELD_EDGE_BOTTOM: 0.0,
            FIELD_EDGE_LEFT: 0.0,
            FIELD_EDGE_RIGHT: 0.0,
        },
    )
    slots = (
        ("brick", FIELD_EDGE_TOP, 1.4, 1.8),
    )

    assert plan_transport_heading(
        FIELD_EDGE_TOP, position_x, 0.0, slots, 0.0
    ) == pytest.approx(expected_heading)


def test_startup_target_ignores_bump() -> None:
    slots = (
        ("bump", FIELD_EDGE_LEFT, 0.45, 0.8),
    )

    assert plan_startup_target_y(slots, 0.7) == pytest.approx(0.7)


def test_return_plan_ignores_bump() -> None:
    slots = (
        ("bump", FIELD_EDGE_BOTTOM, 0.4, 0.6),
    )
...
def test_master_return_plan_uses_negative_relative_y_to_reach_boundary() -> None:
    ...
    slots = (
        _brick(FIELD_EDGE_LEFT, 1.0, 1.4),
    )
...
def test_return_plan_keeps_fixed_retreat_when_safe_edge_is_already_reachable(
    ...
    slots = (
        _brick(FIELD_EDGE_LEFT, 1.05, 1.35),
    )
...
def test_return_plan_retreats_past_right_obstacle_before_turning() -> None:
    ...
    slots = (
        _brick(FIELD_EDGE_RIGHT, 0.8, 1.6),
    )
...
def test_right_edge_return_plan_avoids_large_temporary_candidate_lists() -> None:
    ...
    empty_slots = ()
    obstacle_slots = (
        _brick(FIELD_EDGE_TOP, 1.8, 2.1),
        _brick(FIELD_EDGE_BOTTOM, 1.8, 2.1),
        _brick(FIELD_EDGE_LEFT, 1.05, 1.35),
    )
...
def test_return_plan_avoids_bottom_obstacle_when_selecting_safe_edge_target() -> None:
    ...
    slots = (
        _brick(FIELD_EDGE_BOTTOM, 0.4, 0.6),
    )
...
def test_return_plan_margin_only_expands_obstacle_along_left_edge() -> None:
    ...
    margin_m = 0.2
    slots = (
        _brick(FIELD_EDGE_LEFT, 1.0 + margin_m, 1.4 + margin_m),
    )
...
def test_return_plan_keeps_stage_one_when_margin_closes_safe_edge() -> None:
    ...
    slots = (
        _brick(FIELD_EDGE_LEFT, 0.2, 0.5),
    )
