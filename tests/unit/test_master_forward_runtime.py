...
def test_master_forward_runtime_forwards_velocity_and_keeps_raw_line_for_transport(
    ...
    uart8._buffer = b"vx=1.0,vy=-2.5,omega=3.0\n"
    forward_runtime_module = import_master_module(
        "vision.master.forward_runtime", monkeypatch
    )
    ...
    assert ("handle_uart_line", "uart8", "vx=1.0,vy=-2.5,omega=3.0") in events
...
def test_master_forward_runtime_only_forwards_velocity_fields_from_mixed_command(
    ...
    events, uart8 = install_fake_transport_car(monkeypatch)
    uart8._buffer = b"rear=1,vx=1.0,angle=90,w=-0.5\n"
    forward_runtime_module = import_master_module(
        "vision.master.forward_runtime", monkeypatch
    )
    ...
    assert ("handle_uart_line", "uart8", "rear=1,vx=1.0,angle=90,w=-0.5") in events
...
def test_master_forward_runtime_folds_w_alias_and_keeps_last_angular_velocity(
    ...
    events, uart8 = install_fake_transport_car(monkeypatch)
    uart8._buffer = b"vx=1.0,w=-0.5,omega=2.0\n"
    forward_runtime_module = import_master_module(
        "vision.master.forward_runtime", monkeypatch
    )
    ...
    assert ("handle_uart_line", "uart8", "vx=1.0,w=-0.5,omega=2.0") in events
...
def test_master_forward_runtime_does_not_forward_query_or_non_velocity_command(
    ...
    assert ("handle_uart_line", "uart8", "rear=1,angle=90") in events
...
def test_master_forward_runtime_skips_invalid_velocity_forward_but_records_error(
    ...
    events, uart8 = install_fake_transport_car(monkeypatch)
    uart8._buffer = b"vx=oops,rear=1\n"
    forward_runtime_module = import_master_module(
        "vision.master.forward_runtime", monkeypatch
    )
    ...
    assert uart8.messages == []
    assert ("handle_uart_line", "uart8", "vx=oops,rear=1") in events
...
def test_master_forward_runtime_keeps_running_when_forward_write_fails(
    ...
    assert keep_running is False
    assert ("handle_uart_line", "uart8", "vx=1.0") in events
    assert events[-1] == "transport_step"
...
def test_master_forward_runtime_rejects_non_finite_velocity_field(
    ...
    assert uart8.messages == []
    assert ("handle_uart_line", "uart8", "vx=nan,rear=1") in events
...
def test_master_forward_runtime_rejects_out_of_range_velocity_field(
    ...
    events, uart8 = install_fake_transport_car(monkeypatch)
    uart8._buffer = b"vx=1001,rear=1\n"
    forward_runtime_module = import_master_module(
        "vision.master.forward_runtime", monkeypatch
    )
    ...
    assert uart8.messages == []
    assert ("handle_uart_line", "uart8", "vx=1001,rear=1") in events
