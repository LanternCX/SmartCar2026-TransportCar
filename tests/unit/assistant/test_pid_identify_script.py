def test_pid_identify_script_supports_direct_board_imports() -> None:
    ...
    content = Path("src/assistant/script/pid_identify.py").read_text(encoding="utf-8")
...
def test_format_ident_text_writes_gain_tau_lines() -> None:
    from assistant.script.pid_identify import format_ident_text
...
def test_build_ident_filter_bank_uses_legacy_filter_params(monkeypatch) -> None:
    import assistant.script.pid_identify as script
...
def test_pid_identify_uses_legacy_encoder_mapping() -> None:
    from assistant.script.pid_identify import LEGACY_ENCODER_PINS
...
def test_pid_identify_uses_legacy_motor_mapping() -> None:
    from assistant.script.pid_identify import LEGACY_MOTOR_PORTS
...
def test_resolve_ident_plan_matches_legacy_default_run() -> None:
    from assistant.script.pid_identify import resolve_ident_plan
