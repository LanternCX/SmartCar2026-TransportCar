...
def test_velocity_short_packet_fills_default_omega() -> None:
    consume_result, parsed, passthrough = split_velocity_line("v,12,-4")
...
def test_velocity_short_packet_preserves_omega_presence() -> None:
    consume_result, parsed, passthrough = split_velocity_line("v,6.5,8,1.5")
...
def test_invalid_velocity_short_packet_does_not_produce_velocity_vector() -> None:
    consume_result, parsed, passthrough = split_velocity_line("v,7,bad")
...
def test_key_value_velocity_text_is_not_formal_velocity_input() -> None:
    consume_result, parsed, passthrough = split_velocity_line("vx=-2,vy=3,omega=4")

    assert consume_result == CONSUME_IGNORED
    assert parsed is None
    assert passthrough == "vx=-2,vy=3,omega=4"
