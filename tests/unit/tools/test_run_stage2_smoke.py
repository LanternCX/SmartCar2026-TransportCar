...
def test_main_supports_explicit_assistant_source_dir(monkeypatch):
    """命令行应显式接受辅车运行根目录."""
    ...
    exit_code = run_stage2_smoke.main(
        ["--port", "/dev/ttyUSB0", "--source-dir", "src/assistant"]
    )

    assert exit_code == 0
    assert seen == {"port": "/dev/ttyUSB0", "source_dir": "src/assistant"}


def test_build_probe_commands_include_explicit_source_dir_selector():
    """命令链应显式携带运行根目录选择。"""

    commands = run_stage2_smoke.build_probe_commands(
        "/dev/ttyUSB0", source_dir="src/assistant"
    )
    ...
    for command in commands:
        ...
        assert command[command.index("--source-dir") + 1] == "src/assistant"
...
def test_run_probe_uses_explicit_source_dir_commands(monkeypatch):
    ...
    result = run_stage2_smoke.run_probe("/dev/ttyUSB0", source_dir="src/assistant")

    assert result.status == "ok"
    assert len(commands) == 5
    assert all("--source-dir" in command for command in commands)
    assert all(
        command[command.index("--source-dir") + 1] == "src/assistant"
        for command in commands
    )
