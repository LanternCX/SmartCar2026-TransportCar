...
- OpenART Vision assistant 通过辅车 `UART6` 输出 `v,<vx>,<vy>`，不输出 `omega`；在找物体模式下通过 `r,<seq>,<event>,<value>` 按配置回报 `TARGET_FOUND` 或 `ALIGNED`。搬运收尾阶段的后退段与前进段完成回报 `CLEARED` 由辅车 RT1021 本地生成，不依赖 OpenART Vision assistant。
