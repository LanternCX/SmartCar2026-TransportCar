...
- OpenART Vision assistant 通过辅车 `UART6` 输出 `v,<vx>,<vy>`，不输出 `omega`；在找物体模式下通过 `r,<seq>,<event>,<value>` 按配置回报 `TARGET_FOUND` 或 `ALIGNED`。搬运收尾阶段的后退段与保留的前进段完成回报 `CLEARED` 由辅车 RT1021 本地生成，不依赖 OpenART Vision assistant。
...
OpenART Vision master 在主车绕行修正 hook 下继续识别红色目标物体, 每帧输出 `v,<vx>,<vy>` 平移修正量。

- 绕行修正使用独立的横向和纵向速度参数。
- 无有效目标时输出 `v,0,0`。
- 绕行修正模式只输出速度数据流, 不回报 `TARGET_FOUND`、`ALIGNED` 或 `ARRIVED` 可靠事件。
- 车端主车在 `ORBITING` 中把该平移修正量独立解算为三轮修正目标后叠加到底盘三轮目标速度。
- 绕行修正不改变目标角度、半径倍率、角速度来源和绕行完成判据。
...
OpenART Vision assistant 在 `ASSISTANT_ORBIT` 下执行绕行修正任务。辅车 RT1021 通过本车 `UART6` 下发 `s,<seq>,3,1,<arg>`, OpenART 确认后切换到绕行修正模式。

- 检测任务识别红色目标物体色块。
- 速度计算使用独立的横向和纵向 P 环参数。
- 输出固定为 `v,<vx>,<vy>`, 不输出 `omega`。
- 无有效目标时输出 `v,0,0`。
- 绕行修正模式只输出速度数据流, 不回报 `TARGET_FOUND` 或 `ALIGNED` 可靠事件。
- 车端辅车在 `ASSISTANT_ORBIT` 中把该平移修正量独立解算为三轮修正目标后叠加到底盘三轮目标速度。
- 辅车绕行期间不使用 `UART8` 前馈速度。

## 角色链路

- 角色链路按 `vision/master/` 与 `vision/assistant/` 角色层组织角色运行入口。
- `vision/master/` 是本仓库的主车 RT1021 角色运行入口，不是 OpenART Vision master。
- 主车 RT1021 角色运行入口负责接管本车 `UART6` 视觉速度输入，并按 [串口通信协议](protocol.md) 中的 `UART8` 主辅前馈链路持续转发当前底盘速度目标给辅车，同时写入主车共享底盘执行。
- 主车 RT1021 角色运行入口负责 `UART8` 当前底盘速度转发、本车 `UART6` 主车视觉 hook 通信、主车视觉 `v` 搜索速度接入和主车搜索状态机调度。
...
- 辅车在 `ASSISTANT_ORBIT` 中停止使用 `UART8` 前馈, 只使用本地 `UART6` 绕行视觉修正量生成三轮修正目标并叠加到底盘三轮目标速度。
- 视觉代码可以按设备侧分别维护，但角色入口统一在 `vision/master/` 与 `vision/assistant/` 下；其中主车 RT1021 角色运行入口承担车间转发和主车视觉 hook 编排职责。
...
- 主车承担视觉相关决策与任务状态切换主线；主车 RT1021 角色运行入口负责 `UART8` 当前底盘速度转发、本车 `UART6` 视觉 hook 通信、主车视觉 `v` 搜索速度接入和主车搜索状态机调度。
...
- 辅车角色层在 `ASSISTANT_ORBIT` 中向本地视觉同步绕行修正任务, 本地视觉确认后只使用 `UART6` 绕行修正速度, 不叠加 `UART8` 前馈。
