...
- 由当前处理对象可直接得到的派生信息, 例如 `image_width`、`image_height`、基于当前帧缓存的中间结果, 不再作为参数层层下传。
- 运行态中全局只维护一份的状态, 例如当前 task、当前事件、当前帧缓存、当前图像尺寸和当前 YOLO 候选, 统一放在各自入口的 `state` 对象中读取。
- 物体任务的 OpenART 入口在本地运行态内维护短期跟踪状态, 由 YOLO 低频重定位与 ROI 短期跟踪共同产出统一目标观测, 不把跟踪状态扩展到 RT1021 车端协议。
- 物体任务进入 YOLO 慢帧前, OpenART 通过本地视觉控制可靠包请求 RT1021 暂停底盘运动; 暂停确认后运行 YOLO, YOLO 帧不输出速度和事件, 帧后再通过可靠包恢复 UDP 速度控制。
- 主车识别目标后把当前动态阈值随目标同步到辅车, 辅车目标阶段优先使用同步阈值做色块候选。
- 黄线任务和辅车色标跟随任务继续直接走各自传统算法, 不进入物体 YOLO 跟踪链路。
- 接口收口优先级是: 保留操作对象显式传入, 删除重复派生参数, 再对齐主辅两侧同层 helper 的签名和职责。
