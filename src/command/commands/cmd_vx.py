"""
@file cmd_vx.py
@brief vx 速度指令处理器, 车体系 X 方向速度目标(脉冲/s)
"""
...
# @brief X 方向最大速度限值(脉冲/s), 从配置文件读取, 确保电机指令不超范围
...
def handle(ctx, value):
    """
    @brief 设置车体系 X 方向速度目标(脉冲/s), 进入速度模式并清除位置指令

    @details
    将限幅后的速度值存储到 ctx.last_cmd["vx"], 接管任何旧的 X 速度指令
    ...
    @param value 速度值(脉冲/s), 自动限幅到 [-V_CMD_MAX, V_CMD_MAX]
                 正值前进, 负值后退
    """
    ctx.last_cmd["vx"] = clamp(value, -V_CMD_MAX, V_CMD_MAX)
