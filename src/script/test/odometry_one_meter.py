...
@file src/script/test/odometry_one_meter.py
@brief 让车辆按当前里程计执行一次车体系 y+ 位移动作, 用于现场测量前进真实距离
"""
...
def _log_final_state(car, result, log_fn):
    ...
    target_x = 0.0
    target_y = float(TARGET_DISTANCE_M)
    err_x = target_x - final_x
    err_y = target_y - final_y
    detail = (
        "result=%s target_y=%.3f x=%.3f y=%.3f err_x=%.3f err_y=%.3f"
        % (result, target_y, final_x, final_y, err_x, err_y)
    )
    if log_fn is not None:
        log_fn("odometry_one_meter", detail)
        return
    print("odometry_one_meter: %s" % detail)


def run_one_meter_calibration(
    car,
    sleep_ms=_sleep_ms,
    now_ms=_ticks_ms,
...
def main():
    ...
    try:
        ...
        return run_one_meter_calibration(car, log_fn=log)
    except Exception as exc:
        log_exception("odometry_one_meter", "fatal error: %s" % exc, exc)
        if pit1 is not None:
            pit1.stop()
        if car is not None:
            car.stop()
