...
- maintainer 在 review 当前改动时, 应该能只靠注释和当前代码, 大致明白这段逻辑在做什么
- 如果读完注释和当前代码后, 仍然说不清这一段的作用、边界或这样写的原因, 就说明注释还不够好
...
- 注释与文档字符串统一使用中文
- 文档注释统一采用 Doxygen 风格
- 文档注释不要强调 `最小`、`基线` 这类设计阶段口径, 优先说明文件、类、函数当前承担的职责和作用
- 注释标点统一使用半角符号并保持空格规则, 单行注释行尾不加收尾标点

## 显式注释要求

- 所有变量定义都需要显式注释其角色、来源或用途
- 所有常量定义都需要显式注释其语义、单位或边界含义
