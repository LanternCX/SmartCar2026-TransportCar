...
- 板端入口与主线运行时代码不得依赖 `os.path`, 路径处理必须使用板端可用的最小字符串逻辑
...
- `seekfree_demo/v3.0.0/` 若与旧版 demo 冲突, 默认优先以 `v3.0.0` 为准；对 `IMU660RX` 与 `encoder` 的 `get()` 读取, 必须先按新 demo 建立 `ticker.capture_list(...)` 采样触发链
- 主线运行时代码中的包内依赖与硬件模块依赖都必须直接导入, 不保留任何 `ImportError` / `ModuleNotFoundError` 兜底、假对象或默认值回退
- 参数与阈值统一集中到 `config/params.py`
- 禁止静默失败, 只捕获预期异常并保留上下文

## LSP / Pyright 排障约束

- 若某个 `src/` 包内导入在运行时正常、但 Pyright 报 `unknown import symbol`, 先检查仓库根目录是否存在同名包遮蔽 `src/` 下真实模块, 不要先怀疑业务代码缺成员
- 遇到这类同名包遮蔽时, 优先改为 `from config import params as _params` 这类模块级导入再用 `getattr(...)` 取值, 先消除错误解析来源, 不要为了消红改坏板端导入路径
- 对 `execfile` 这类板端提供的全局能力, 优先保持运行时代码写法不变; 若仅是 Pyright 误报, 使用单行局部忽略, 不要新增 `builtins.pyi` 去覆盖整套内建声明
