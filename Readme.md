...
当前仓库默认采用“双车同仓库, 按角色裁剪能力”的方式运行。因此部署和联调时，必须先明确每台车的车号、角色和拨码配置。

建议默认配置如下：

| 车号 | 角色 | D8 | D9 | 正常运行能力 |
| :--- | :--- | :--- | :--- | :--- |
| **1 号车** | **主车** | `1` | `0` | 开启视觉处理、双摄轮询和唯一任务状态机 |
| **2 号车** | **辅车** | `0` | `1` | 关闭视觉处理, 仅保留执行与状态回传 |

补充说明：

- `D8=1,D9=0` 表示主车
- `D8=0,D9=1` 表示辅车
- `D8/D9=0/0` 或 `1/1` 为非法组合, 启动时应安全失败
- 若现场贴号与上表不同, 请先同步修改本文档和对应 HIL 记录, 不要只在口头上约定
...
- OpenArt 端回传完整识别框：`left=<pixel_left>,top=<pixel_top>,right=<pixel_right>,bottom=<pixel_bottom>`。
- 仅当 `UART6` 包中恰好只有 `left,top,right,bottom` 四个键时，才进入视觉状态机。
- `UART6` 上凡是包含视觉字段名 `x/y/left/top/right/bottom` 的旧载荷或不完整载荷，都会先被视觉协议吞掉，不再落入普通命令路由。
- 若需要发送绝对位置命令，请使用 `UART3`；`UART6` 常态上保留给视觉链路。
- 其余 `UART6` 命令仍走现有路由协议，例如 `vx=...`、`reset`、`?lock`。
- 视觉状态机内部会从识别框派生 `center_x`、`center_y`、`bottom` 等几何量，再把像素误差转成相对控制意图，由主控位置环输出，避免高帧率下直接重复发送外部增量命令造成累加过冲。
...
# UART6 视觉输入：OpenArt 连续回传完整识别框
left=148,top=24,right=188,bottom=96
